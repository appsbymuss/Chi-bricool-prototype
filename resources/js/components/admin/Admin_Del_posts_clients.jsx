...
const Admin_Del_posts_clients = () => {
    ...
    const fetchPostsClients = async () =>{

        try {
            const response = await axios.get(`/api/view_post_clientt/${id.id}`);
    ...
    const deletepost  = (idp) =>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const res = await axios.delete(`/api/delet_post_client/${idp}`);
                    fetchPostsClients()
                    Swal.fire(
                        'Deleted!',
                        res.data.message,
  ...
  return (
    <>
    <Link to={'/Admin_view_delete_clients'}><button style={{'borderRadius':'5px','padding':'10px','fontWeight':'bold','background':'#66b3ff','border':'none','float':'right','marginTop':'-50px','marginRight':'90px'}}>Posts Clients</button></Link>
...
export default Admin_Del_posts_clients
